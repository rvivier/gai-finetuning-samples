...
def training_function(args):
    ...
    #we're adding custom model/predict methods in inference.py to add the trust remode code =True notably.
    shutil.copyfile(
    ...
    #the next 2 files are required for inference and are taken from the falcon7B model downloaded from HF.
    shutil.copyfile(
        os.path.join(os.path.dirname(__file__), "configuration_RW.py"),
        "/opt/ml/model/configuration_RW.py",
    )
    shutil.copyfile(
        os.path.join(os.path.dirname(__file__), "modelling_RW.py"),
